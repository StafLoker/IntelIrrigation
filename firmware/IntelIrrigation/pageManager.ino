...
uint32_t convertScheduleToMs() {
  uint32_t daysInSeconds = (uint32_t)configuration.schedule.days * 24 * 60 * 60;
  uint32_t hoursInSeconds = (uint32_t)configuration.schedule.hours * 60 * 60;
  uint32_t minutesInSeconds = (uint32_t)configuration.schedule.mins * 60;
  return (daysInSeconds + hoursInSeconds + minutesInSeconds) * 1000;
}